...
function OnGUI () {
    if (GUI.Button(Rect(0, 0, 80, 20), "Reset car")) {
        var car : Transform = (GameObject.Find("Car")).transform;
        car.position.y = 0;
...
/**
        ...
        var hit : WheelHit;
/**
         * @typedef {WheelCollider} WheelCollider
         */
        ...
        if (c.GetGroundHit(hit)) {
            ...
            if (slip>0.5  any.collider.tag=="Dusty"); {
...
function HandlePhysics () {
    var velo : Vector3 = rigidbody.velocity;
    ...
    var flatDir : Vector3 = Vector3.Normalize(new Vector3(dir.x,0,dir.z));
    var flatVelo : Vector3 = new Vector3(velo.x,0,velo.z);
    ...
    var engineForce : Vector3 = flatDir*CalcEngine();
    var totalbrake : float = brake + handbrake*0.5;
    if (totalbrake > 1.0) totalbrake = 1.0;
    var brakeForce : Vector3 = -flatVelo.normalized*totalbrake*rigidbody.mass*maxBrakeAccel;
 
    flatDir *= flatVelo.magnitude;
    flatDir = Quaternion.AngleAxis(steer*30.0,Vector3.up)*flatDir;
    flatDir *= rev;
    var diff : float = (flatVelo-flatDir).magnitude;
    var cornerAccel : float = maxCornerAccel;
    if (cornerAccel > diff) cornerAccel = diff;
    var cornerForce : Vector3 = -(flatVelo-flatDir).normalized*cornerAccel*rigidbody.mass;
    ...
    var handbrakeFactor : float = 1 + handbrake*4;
   
    if (rev<0)
        handbrakeFactor = 1;
    var veloSteer : float = ((15/(2*velo.magnitude+1))+1)*handbrakeFactor;
    var steerGrip : float = (1-handlingTendency*cornerSlip);
    if (rev*steer*steerVelo < 0)
        steerGrip=1;
    var maxRotSteer : float = 2*Time.fixedDeltaTime*handbrakeFactor*steerGrip;
    var fVelo : float = velo.magnitude;
    var veloFactor : float = fVelo < 1.0 ? fVelo : Mathf.Pow(velo.magnitude,0.3);
    var steerVeloInput : float = rev*steer*veloFactor*0.5*Time.fixedDeltaTime*handbrakeFactor;
